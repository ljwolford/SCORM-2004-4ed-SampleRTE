...
public class SQLiteJDBC{
    public static void main(String args[]){
        ...
        try{
            ...
            sqlStuff1.add("CREATE TABLE IF NOT EXISTS CourseInfo(CourseID CHAR(50) PRIMARY KEY,CourseTitle CHAR(200),ImportDateTime CHAR(50),Active INTEGER NOT NULL CHECK(Active IN (0,1)),Start INTEGER NOT NULL CHECK(Start IN (0,1)),TOC INTEGER NOT NULL CHECK(TOC IN (0,1)));");
            sqlStuff1.add("CREATE TABLE IF NOT EXISTS ItemInfo(ActivityID INTEGER PRIMARY KEY AUTOINCREMENT,CourseID CHAR(255),OrganizationIdentifier CHAR(255),ItemIdentifier CHAR(255),ResourceIdentifier CHAR(255),Launch LONGTEXT,Type CHAR(50),Title CHAR(255),ParameterString LONGTEXT,DataFromLMS LONGTEXT,MinNormalizedMeasure CHAR(50),AttemptAbsoluteDurationLimit CHAR(255),TimeLimitAction CHAR(255),CompletionThreshold CHAR(255),Next INTEGER NOT NULL CHECK(Next IN (0,1)),Previous INTEGER NOT NULL CHECK(Previous IN (0,1)),Exit INTEGER NOT NULL CHECK(Exit IN (0,1)),ExitAll INTEGER NOT NULL CHECK(ExitAll IN (0,1)),Abandon INTEGER NOT NULL CHECK(Abandon IN (0,1)),Suspend INTEGER NOT NULL CHECK(Suspend IN (0,1)));");
